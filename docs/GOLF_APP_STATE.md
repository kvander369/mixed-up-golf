...
> card, and in use. Live at v21. Seven test suites, 99 checks, all passing.
> Nothing blocking.
...
| `nassau_test.js` | The Nassau settles the way the group plays it; runs the app's real function — 23 checks |
...
press rows, money per nine beside its name, no grey section headers, `# Golf app — living state
...
line you consult once a week has to be legible when you consult it.
 ...
 on
the stakes, the carry switch only when the front is actually tied. The old
"take it 5–4" block on Results is gone - *"the new game trumps all that"* -
and the Holes grid's Out/In show the bets on that nine ("4-2-0") instead of
holes won.
...
**Two lessons for the record.** The browser check matters: none of the suites
can see CSS, and twice today a broken comment passed every test. And Kyle
reads without glasses, outdoors - when he says something is hard to see, go
decisively bigger and white; nudging costs a deploy each time.
